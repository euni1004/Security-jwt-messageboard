...
public class PostService {
    ...
    public Post createPost(PostRequestDto requestDto){
        ...
        Post post = new Post(requestDto.getTitle(),requestDto.getContent(), member.getNickname(),member.getMemberId());
    ...
    public List<PostRetunDto> getPosts(){
        ...
        for(Post post : posts){
            Member member = memberRepository.findById(post.getMemberId()).orElseThrow(()-> new IllegalArgumentException("사용자가 존재하지않음"));
            MemberResponseDto postmember =MemberResponseDto.of(member);
    ...
    public PostCommentReturnDto getPost(Long postid){
        ...
        List<Comment> comments = commentRepository.findAllByPostId(post.getPostId()).orElse(null);
